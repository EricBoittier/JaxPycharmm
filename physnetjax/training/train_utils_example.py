# based on an example from Thorben Frank 
# https://github.com/EricBoittier/euclidean_fast_attention/commit/c348a37e2fdb04d4b1c94635da24ab5dadae4a83#diff-2e2af370f7a8ba0009c6dcc806435fa1180ba7ccd1fb95df9f1de6dcda36b834
import jraph
import jax
...
class EvalMetrics(metrics.Collection):
 ...
 dipole_mse: metrics.Average.from_output('forces_mse')
 dipole_dr_mse: metrics.Average.from_output('forces_mse')
...
class TrainMetrics(metrics.Collection):
 ...
 energy_mse: metrics.Average.from_output('energy_mse')
 hessian_mse: metrics.Average.from_output('forces_mse')
 forces_mse: metrics.Average.from_output('forces_mse')
 dipole_dr_mse: metrics.Average.from_output('forces_mse')
 dipole_mse: metrics.Average.from_output('forces_mse')
...
"""
 ...
 def loss_fn(params, batch):
     ...
     dipoles, dipole_dr = output["dipoles"], output["dipole_derivatives"]
     
     energy_label, forces_label = batch['energy'], batch['forces']
     hessian_label = batch["hessian"]
     dipoles_label, dipole_dr_label = batch["D"], batch["dDdR"]
     ...
     hessian_mse = node_mse_loss(
         y=hessian,
         y_label=hessian_label_label,
         batch_segments=batch['batch_segments'],
         graph_mask=graph_mask,
         scale=1.
     )

     dipole_mse = node_mse_loss(
         y=dipoles,
         y_label=dipoles_label,
         batch_segments=batch['batch_segments'],
         graph_mask=graph_mask,
         scale=1.
     )

     dipole_dr_mse = node_mse_loss(
         y=dipole_dr,
         y_label=dipole_dr_label,
         batch_segments=batch['batch_segments'],
         graph_mask=graph_mask,
         scale=1.
     )
     ...
     loss += dipole_dr_mse + dipole_mse
     loss += hessian_mse
     return loss, (energy_mse, forces_mse, dipole_mse, dipole_dr_mse, hessian_mse)
...
def make_train_step_fn(loss_fn, optimizer):
 ...
 def train_step_fn(params, opt_state, batch):
     (loss, (energy_mse, forces_mse, dipole_mse, dipole_dr_mse, hessian_mse)), grad = jax.value_and_grad(
     ...
     mtrcs = {
         ...
         "dipole_dr_mse": dipole_mse,
         "hessian_mse": dipole_mse,
         'grad_norm': grad_norm,
     }
