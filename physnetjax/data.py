...
def prepare_batches(
    ...
    for perm in perms:
        ...
        batch = {
            k: (v[perm].reshape(batch_size * num_atoms) if k in ['Z', 'mono']
                else v[perm].reshape(batch_size * num_atoms, -1) if k in ['R', 'F']
...
def prepare_batches_old(
    ...
    if steps_per_epoch == 0:
        raise ValueError("Batch size is larger than the dataset size or no full batch available.")
    ...
    for perm in perms:
        ...
        if debug_mode:
            ...
            assert batch['R'].shape == (batch_size * num_atoms, 3), f"R shape mismatch: {batch['R'].shape}"
            assert batch['F'].shape == (batch_size * num_atoms, 3), f"F shape mismatch: {batch['F'].shape}"
            assert batch['E'].shape == (batch_size, 1), f"E shape mismatch: {batch['E'].shape}"
            assert batch['Z'].shape == (batch_size * num_atoms,), f"Z shape mismatch: {batch['Z'].shape}"
            assert batch['N'].shape == (batch_size,), f"N shape mismatch: {batch['N'].shape}"
