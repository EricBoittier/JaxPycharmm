...
def mean_squared_loss_QD(
    ...
    forces_loss = jnp.mean(
        optax.l2_loss(forces_prediction.flatten(), forces_target.flatten())
    )
    dipole_loss = jnp.mean(
        optax.l2_loss(dipole_prediction.flatten(), dipole_target.flatten())
    )
    charges_loss = jnp.mean(
        optax.l2_loss(total_charges_prediction.flatten(), total_charge_target.flatten())
    )
